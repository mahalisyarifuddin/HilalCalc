...
A Pareto Frontier analysis was performed to minimize the number of decimal digits in the constants while maintaining maximum accuracy.
...
- `Slope` = 29.53057946 (8 decimal digits)
- `Epoch (Integer)` = 2302456
- `Phase Shift` = -3.3233 (4 decimal digits)
- `Inverse Offset` = 4.3233 (1.0 - Phase Shift, for round-trip consistency)

## Accuracy
- **Range**: 1000 AH to 6000 AH (60000 months).
- **Exact Matches (Month Starts)**: 43503 (72.51%).
- **Obligatory Months Accuracy**: ~72.63% (Ramadan, Shawwal, Dhu al-Hijjah).
- **Comparison**: This formula uses an optimized slope and phase shift to prioritize accuracy for obligatory months over a 5000-year span (1000-6000 AH). The constants were selected from the Pareto optimal set to balance brevity and precision.
