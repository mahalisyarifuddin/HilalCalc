...
// 4D Pareto Frontier Logic
function findParetoFrontier4D(candidates) {
    // Candidates structure: { C, accAll, impAll, accOblig, impOblig }
    let frontier = [];
    ...
    for (const cand of candidates) {
        ...
        for (const other of candidates) {
            ...
            // Does 'other' dominate 'cand'?
            // We want to MAXIMIZE Accuracy and MINIMIZE Impossible Rate.
            // Condition for dominance:
            // other.accAll >= cand.accAll AND
            // other.impAll <= cand.impAll AND
            // other.accOblig >= cand.accOblig AND
            // other.impOblig <= cand.impOblig AND
            // (at least one strict inequality)
            const betterOrEqual = (
                other.accAll >= cand.accAll &&
                other.impAll <= cand.impAll &&
                other.accOblig >= cand.accOblig &&
                other.impOblig <= cand.impOblig
            );
    ...
    // Sort by Total Accuracy descending for readability
    return frontier.sort((a, b) => (b.accAll + b.accOblig) - (a.accAll + a.accOblig));
}

function calculateScore(cand) {
    // Balanced Score: (AccAll - 2*ImpAll) + (AccOblig - 2*ImpOblig)
    return (cand.accAll - 2 * cand.impAll) + (cand.accOblig - 2 * cand.impOblig);
}

async function main() {
    console.log(`Analyzing years ${startYear}-${endYear} AH`);
    console.log('Finding Pareto-optimal C values (4 Dimensions: AccAll, ImpAll, AccOblig, ImpOblig)');
    console.log('---');
    ...
    for (const loc of locations) {
        ...
        const frontier = findParetoFrontier4D(candidates);

        // Find best balanced score
        let bestBalanced = candidates[0];
        let maxScore = -Infinity;
        for (const cand of candidates) {
            const score = calculateScore(cand);
            if (score > maxScore) {
                maxScore = score;
                bestBalanced = cand;
            }
        ...
        console.log("4D Pareto Frontier:");
        frontier.forEach(c => {
            const score = calculateScore(c);
            const isBest = (c.C === bestBalanced.C) ? " [BEST BALANCED]" : "";
            console.log(`  C=${c.C}: AccAll=${c.accAll.toFixed(2)}%, ImpAll=${c.impAll.toFixed(2)}% | AccOblig=${c.accOblig.toFixed(2)}%, ImpOblig=${c.impOblig.toFixed(2)}% (Score: ${score.toFixed(2)})${isBest}`);
        });
